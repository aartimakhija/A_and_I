...
export function Product({ product, related }: { product: SFProduct; related: SFProduct[] }) {
  ...
  const { rm, addToCart, saved, toggleSaved } = useStore();
  const [size, setSize] = useState(product.variants.find((v) => v.stock > 0)?.size ?? SIZES[2]);
    ...
    <>
      ...
      <section style={{ display: "grid", gridTemplateColumns: "1.1fr 1fr", gap: "clamp(24px,4vw,64px)",
        ...
        <div>
          ...
          <div style={{ fontFamily: SANS, fontSize: 11, letterSpacing: 1, color: soldOut ? "#B0503E" : T.gold, marginBottom: 24 }}>
            {soldOut ? "Sold out — join the waitlist" : totalStock <= 5 ? `Only ${totalStock} left` : "In stock"}
          </div>
          {product.story && <p style={{ fontFamily: SANS, fontWeight: 300, fontSize: 15, lineHeight: 1.8, color: T.mid, maxWidth: 460 }}>{product.story}</p>}

          {!soldOut && (
            <>
              <div style={{ margin: "30px 0 8px" }}><Eyebrow>Size</Eyebrow></div>
              <div style={{ display: "flex", gap: 10 }}>
                {SIZES.map((s) => {
                  const stock = sizeStock(s);
                  return (
                    <button key={s} disabled={stock === 0} onClick={() => setSize(s)}
          ...
          {product.tiers.length > 0 && !soldOut && (
            <>
