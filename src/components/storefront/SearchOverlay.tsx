      ...
      >
        <div className="mx-auto max-w-3xl">
          <div className="flex items-center gap-4 border-b border-foreground pb-3">
            ...
            />
            <button onClick={() => setSearchOpen(false)} className="micro text-muted-foreground">Close</button>
          </div>
          {!q && (
            <div className="mt-5.5">
              ...
              <div className="mt-3 flex flex-wrap gap-2.5">
                {popular.map((p) => (
                  <button key={p} onClick={() => setQ(p)} className="border border-border px-4 py-2 text-xs hover:border-foreground">{p}</button>
                ))}
              </div>
            </div>
          )}
          {q && (
            <div className="mt-5.5">
              ...
              <div className="mt-3.5 grid grid-cols-2 gap-4 md:grid-cols-4">
                {results.map((d) => (
                  <button key={d.id} onClick={() => go(d.slug)} className="text-left">
