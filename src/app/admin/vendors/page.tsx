  ...
  return (
    <>
      ...
      <table style={{ width: "100%", borderCollapse: "collapse", marginTop: 16 }}>
        <thead><tr>{["Name", "Email", "Status", "Commission", "MOQ", "Lead time (days)", "Products", "", ""].map((h) => <th key={h} style={{ textAlign: "left", borderBottom: "1px solid #ccc", padding: 8 }}>{h}</th>)}</tr></thead>
        <tbody>{vendors.map((v) => (
          <tr key={v.id}>
            <td style={{ padding: 8 }}>{v.name}</td><td>{v.email}</td><td>{v.status}</td>
            <td>{(v.commission * 100).toFixed(0)}%</td><td>{v.moq}</td>
            <td><VendorLeadTime id={v.id} days={v.leadTimeDays} /></td>
